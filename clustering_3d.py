...
parser.add_argument("-args0", type = str, default = ('D:\\results\dissected_2944l_fatemeh_overhang.nrrd'), help = "dissected image address")
...
if data[0].__len__()>40:
    ...
    if nn<1:
        nn=1
    if scale<=12:
        nn=math.ceil((data[0].__len__()/math.floor(scale)))
        if nn>300:
            nn=300
    if scale<=5:

        nn=math.ceil((data[0].__len__()/math.floor(scale)))/2
        if nn>350:
            nn=350
    if scale<=2:
        nn=math.ceil((data[0].__len__()/math.floor(scale)))/5
        nn=500
        # if nn>500:
        #     nn=500
